...
function SuperSEO(props) {
    ...
    var lang = props.lang;
    var titleElement = title ? React.createElement("title", null, title) : undefined;
    var htmlAttributes = lang ? { lang: lang } : undefined;
    var meta = [];
    if (description) {
        meta.push({
            name: 'description',
    ...
    meta.push.apply(meta, createOpenGraphMetadataObjects(props.openGraph, title, description));
    meta.push.apply(meta, createTwitterMetadataObjects(props.twitter, title, description));
    meta.push.apply(meta, createGoogleMetadataObjects(props.google));
...
function createOpenGraphMetadataObjects(data, fallbackTitle, fallbackDescription) {
    ...
    if (data.ogType) {
        switch (data.ogType) {
            case 'article':
                meta.push.apply(meta, createOpenGraphTypeArticleMetadataObjects(data.ogTypeObject));
                break;
            case 'book':
                meta.push.apply(meta, createOpenGraphTypeBookMetadataObjects(data.ogTypeObject));
                break;
            case 'profile':
                meta.push.apply(meta, createOpenGraphTypeProfileMetadataObjects(data.ogTypeObject));
                break;
            case 'website':
            ...
            default:
                ...
                if (data.ogTypeObject) {
                    var metaPropsFunction = data.ogTypeObject;
                    if (typeof metaPropsFunction === 'function') {
                        meta.push.apply(meta, metaPropsFunction());
                    }
                }
                break;
        }
    ...
    // og:image
    if (data.ogImage) {
        var images = Array.isArray(data.ogImage) ? data.ogImage : [data.ogImage];
        images.forEach(function (image) {
            meta.push.apply(meta, createOpenGraphImageMetadataObjects(image));
        });
    } // END og:image
    // og:url
    if (data.ogUrl) {
    ...
    // og:audio
    if (data.ogAudio) {
        var audios = Array.isArray(data.ogAudio) ? data.ogAudio : [data.ogAudio];
        audios.forEach(function (audio) {
            meta.push.apply(meta, createOpenGraphAudioMetadataObjects(audio));
        });
    } // END og:audio
    // og:description
    ...
    if (data.ogLocaleAlternate) {
        data.ogLocaleAlternate.forEach(function (localeAlternate) {
            meta.push({
    ...
    // og:video
    if (data.ogVideo) {
        var videos = Array.isArray(data.ogVideo) ? data.ogVideo : [data.ogVideo];
        videos.forEach(function (video) {
            meta.push.apply(meta, createOpenGraphVideoMetadataObjects(video));
        });
    } // END og:video
    // fb:app_id
    if (data.fbAppId) {
...
function createOpenGraphImageMetadataObjects(image) {
    ...
    if (image.ogImage) {
        ...
        if (image.ogImageType) {
            ...
            for (var i = 0; i < mimeTypeKeys.length; i++) {
                var extension = mimeTypeKeys[i];
...
function createOpenGraphTypeArticleMetadataObjects(article) {
    ...
    if (article.articleAuthor) {
        article.articleAuthor.forEach(function (authorUri) {
            meta.push({
    ...
    if (article.articleTag) {
        article.articleTag.forEach(function (tag) {
            meta.push({
...
function createOpenGraphTypeBookMetadataObjects(book) {
    ...
    // book:author
    if (book.bookAuthor) {
        book.bookAuthor.forEach(function (authorUri) {
            meta.push({
    ...
    // book:tag
    if (book.bookTag) {
        book.bookTag.forEach(function (tag) {
            meta.push({
                property: 'book:tag',
                content: tag,
            });
...
function createTwitterMetadataObjects(data, fallbackTitle, fallbackDescription) {
    ...
    if (data.twitterSummaryCard) {
        meta.push.apply(meta, createTwitterSummaryCardMetadataObjects(data.twitterSummaryCard, fallbackTitle, fallbackDescription));
    }
    if (data.twitterSummaryCardWithLargeImage) {
        meta.push.apply(meta, createTwitterSummaryCardWithLargeImageMetadataObjects(data.twitterSummaryCardWithLargeImage, fallbackTitle, fallbackDescription));
    }
    if (data.twitterPlayerCard) {
        meta.push.apply(meta, createTwitterPlayerCardMetadataObjects(data.twitterPlayerCard, fallbackTitle, fallbackDescription));
    }
    if (data.twitterAppCard) {
        meta.push.apply(meta, createTwitterAppCardMetadataObjects(data.twitterAppCard, fallbackTitle, fallbackDescription));
    }
    return meta;
}
